...
//
static const char scope_id[] = "0ne00022545";
...
{
    if (iothub_authenticated && (iothub_client_handle != NULL))
        return true;
    ...
    AZURE_SPHERE_PROV_RETURN_VALUE provResult =
        IoTHubDeviceClient_LL_CreateWithAzureSphereDeviceAuthProvisioning(scope_id, 10000,
    ...
    if (IoTHubDeviceClient_LL_SetOption(iothub_client_handle, "TrustedCerts",
                                        AzureIoTCertificatesX) != IOTHUB_CLIENT_OK) {
        LogMessage("ERROR: failure to set option \"TrustedCerts\"\n");
        return false;
    }

    if (IoTHubDeviceClient_LL_SetOption(iothub_client_handle, OPTION_KEEP_ALIVE,
    ...
    IoTHubDeviceClient_LL_SetMessageCallback(iothub_client_handle, receiveMessageCallback, NULL);
    IoTHubDeviceClient_LL_SetDeviceMethodCallback(iothub_client_handle, directMethodCallback, NULL);
    IoTHubDeviceClient_LL_SetDeviceTwinCallback(iothub_client_handle, twinCallback, NULL);
...
void AzureIoT_SendMessage(const char *messagePayload)
{
    if (iothub_client_handle == NULL) {
        ...
        if (message_handle == 0) {
            ...
            if (IoTHubDeviceClient_LL_SendEventAsync(iothub_client_handle, message_handle,
                                                     sendMessageCallback,
                ...
                if (JSONSuccess ==
                    ...
                    if (reportedPropertiesString != NULL) {
                        if (IoTHubDeviceClient_LL_SendReportedState(
                                iothub_client_handle, (unsigned char *)reportedPropertiesString,
                                strlen(reportedPropertiesString), reportStatusCallback,
                                0) != IOTHUB_CLIENT_OK) {
                            LogMessage("ERROR: failed to set reported state for '%s'.\n",
                                       propertyName);
                        } else {
                            LogMessage("INFO: Reported state for '%s' to value '%d'.\n",
                    ...
                    LogMessage(
                        "ERROR: could not set the property value for Device Twin reporting.\n");
                }
            } else {
...
/// <param name="context">The user context specified at IoTDeviceHubClient_LL_SetMessageCallback()
                                        ...
                                        void *userContextCallback)
{
    ...
    if (hubConnectionStatusCb) {
        hubConnectionStatusCb(iothub_authenticated);
    }
